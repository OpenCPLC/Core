  ...
  uint8_t fill_zero, uint8_t fill_space)
{
  ...
  // zero padding (exclude space for '-')
  while(n < (fill_zero - (is_negative ? 1 : 0))) str[n++] = '0';
  if(is_negative)  str[n++] = '-'; // add sign if needed
  while(n < fill_space) str[n++] = ' '; // space padding to minimum width
  return n;
}

// Padding parameters are `uint8_t`, so one call writes at most 255 bytes.
// Not terminated, callers take the returned length.
char StrTempMem[256];
 ...
 * Requires prior validation with `str_is_uf32`.
 * @param str Input string.
 ...
 * Owned by the garbage collector and released by `heap_clear()`, never by the caller.
