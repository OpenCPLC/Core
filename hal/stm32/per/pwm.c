...
// Advanced and general-purpose timers with a break register: outputs gated by `MOE`
static bool has_bdtr(TIM_TypeDef *reg)
{
...
static void irq_handler(PWM_t *pwm)
{
...
// PWM mode 1 with preload on a channel, then its enable and polarity bits in `CCER`
static void channel_init(PWM_t *pwm, TIM_Channel_t channel)
{
...
static void channels_init(PWM_t *pwm)
{
  pwm->_ccer_mask = 0;
...
void PWM_Init(PWM_t *pwm)
{
  ...
  if(pwm->UpdateCallback) {
    IRQ_EnableTIM(pwm->reg, pwm->irq_priority, (IRQ_Handler_t)irq_handler, pwm);
    pwm->reg->DIER |= TIM_DIER_UIE;
  }
  if(has_bdtr(pwm->reg)) pwm->reg->BDTR |= TIM_BDTR_MOE;
...
void PWM_SetDeadtime(PWM_t *pwm, uint16_t deadtime)
{
  ...
  if(deadtime < 128) {
    ...
    step = (deadtime - 128) / 2;
    dtg = (0b10 << 6) | step;
    pwm->deadtime = (uint16_t)step * 2 + 128;
  }
  else if(deadtime < 512) {
    step = (deadtime - 256) / 8;
    dtg = (0b110 << 5) | step;
    pwm->deadtime = (uint16_t)step * 8 + 256;
  }
  else {
    step = (deadtime - 512) / 16;
    dtg = (0b111 << 5) | step;
    pwm->deadtime = (uint16_t)step * 16 + 512;
  }
...
void PWM_SetAlign(PWM_t *pwm, PWM_Align_t align)
{
  ...
  pwm->reg->CNT = 0;
  pwm->align = align;
  pwm->reg->CR1 = (pwm->reg->CR1 & ~TIM_CR1_CMS_Msk) | ((uint32_t)align << TIM_CR1_CMS_Pos);
  pwm->reg->CR1 |= TIM_CR1_CEN;
}
...
void PWM_InterruptEnable(PWM_t *pwm) { pwm->reg->DIER |= TIM_DIER_UIE; }
void PWM_InterruptDisable(PWM_t *pwm) { pwm->reg->DIER &= ~TIM_DIER_UIE; }
...
void PWM_Trigger(PWM_t *pwm, TIM_Channel_t channel, uint16_t compare, bool irq)
{
  ...
  *ccmr = (*ccmr & ~(0x7u << pos)) | (0x7u << pos);
  ...
  pwm->reg->CR2 = (pwm->reg->CR2 & ~TIM_CR2_MMS2_Msk) | ((0x4u + channel) << TIM_CR2_MMS2_Pos);
