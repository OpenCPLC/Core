...
static inline uint32_t rtc_alarm_wf_mask(RTC_Alarm_e alarm)
{
  return (alarm == RTC_Alarm_A) ? RTC_ISR_ALRAWF : RTC_ISR_ALRBWF;
}

static inline void rtc_clear_isr(uint32_t flags)
{
  rtc_unlock();
  RTC->ISR &= ~flags;
  rtc_lock();
...
void RTC_SetDatetime(RTC_Datetime_t *datetime)
{
  ...
  while(!(RTC->ISR & RTC_ISR_INITF)) __DSB();
  RTC->TR = tr;
  RTC->DR = dr;
  RTC->ISR &= ~RTC_ISR_INIT;
  while(!(RTC->ISR & RTC_ISR_RSF)) __DSB();
  rtc_lock();
...
void RTC_AlarmEnable(RTC_Alarm_e alarm, const RTC_Alarm_t *cfg)
{
  ...
  rtc_unlock();
  RTC->CR &= ~mask;
  while(!(RTC->ISR & rtc_alarm_wf_mask(alarm))) __DSB();
  *rtc_alarm_reg(alarm) =
...
void RTC_Alarm_IRQHandler(void)
{
  if(RTC->ISR & RTC_ISR_ALRAF) {
    rtc_clear_isr(RTC_ISR_ALRAF);
    rtc_flags.alarm_a = true;
  }
  if(RTC->ISR & RTC_ISR_ALRBF) {
    rtc_clear_isr(RTC_ISR_ALRBF);
