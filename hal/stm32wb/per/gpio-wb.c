...
void GPIO_BackendWakeup(GPIO_t *gpio)
{
  ...
  if(gpio->wakeup_pull == GPIO_WakeupPull_Up) {
    switch((uint32_t)gpio->port) {
      ...
      case (uint32_t)GPIOE: PWR->PUCRE |= (1U << gpio->pin); break;
      case (uint32_t)GPIOH: PWR->PUCRH |= (1U << gpio->pin); break;
      default: break;
    }
  }
  else {
    switch((uint32_t)gpio->port) {
      ...
      case (uint32_t)GPIOE: PWR->PDCRE |= (1U << gpio->pin); break;
      case (uint32_t)GPIOH: PWR->PDCRH |= (1U << gpio->pin); break;
      default: break;
    }
  }
}
...
{
  if(EXTI->PR1 & (1U << exti->pin)) {
    EXTI->PR1 = (1U << exti->pin); // Write 1 to clear (don't use |=)
    bool level = ((exti->port->IDR >> exti->pin) & 0x01U) != 0;
    exti->state = level;
    if(level) exti->rise_events++;
    else exti->fall_events++;
    exti->int_cnt++;
    if(exti->int_function) exti->int_function(exti->int_struct);
...
void EXTI_Init(EXTI_t *exti)
{
  ...
  uint32_t exticr_move = 4 * (exti->pin % 4);
  uint32_t exticr = SYSCFG->EXTICR[exticr_reg];
  ...
  switch((uint32_t)exti->port) {
    ...
    case (uint32_t)GPIOE: exticr |= (4U << exticr_move); break;
    case (uint32_t)GPIOH: exticr |= (7U << exticr_move); break;
    default: break;
  }
  SYSCFG->EXTICR[exticr_reg] = exticr;
  ...
  IRQ_EnableEXTI(exti->pin, exti->irq_priority, exti->irq_subpriority,
                 (void (*)(void *))&EXTI_Interrupt, exti);
}
