...
// Buffer descriptor words: `TXBD` = count `<< 16` | address,
// `RXBD` = block field | received count `<< 16` | address
#define PMA_TX_COUNT_Pos 16
#define PMA_RX_COUNT_Pos 16
#define PMA_COUNT_MASK   0x03FFu

static inline volatile uint32_t *pma(uint16_t offset) {
  return (volatile uint32_t *)(USB_DRD_PMAADDR + offset);
}

static void pma_write(uint16_t offset, const uint8_t *src, uint16_t len)
{
  volatile uint32_t *dst = pma(offset);
  while(len >= 4) {
    *dst++ = (uint32_t)src[0] | ((uint32_t)src[1] << 8) |
  ...
  if(len) {
    uint32_t word = 0;
    for(uint16_t i = 0; i < len; i++) word |= (uint32_t)src[i] << (8 * i);
    *dst = word;
  }
}
...
{
  volatile uint32_t *src = pma(offset);
  while(len >= 4) {
    uint32_t word = *src++;
    *dst++ = (uint8_t)word;
    *dst++ = (uint8_t)(word >> 8);
  ...
  if(len) {
    uint32_t word = *src;
    for(uint16_t i = 0; i < len; i++) dst[i] = (uint8_t)(word >> (8 * i));
  }
}

// `RXBD` block field: 2-byte blocks up to 62, 32-byte blocks above
static uint32_t pma_rx_blocks(uint16_t size)
{
  if(size <= 62) return (uint32_t)(size / 2) << 26;
  return 0x80000000u | ((uint32_t)(size / 32 - 1) << 26);
}

//------------------------------------------------------------------------------ Endpoint registers

// `CHEPnR` mixes plain bits, toggle-on-write status fields and clear-on-write-0 flags.
...
static inline volatile uint32_t *epr(uint8_t ep) { return &USB_DRD_FS->CHEP0R + ep; }

static void ep_toggle(uint8_t ep, uint32_t field, uint32_t value)
{
  volatile uint32_t *reg = epr(ep);
  *reg = ((*reg & (USB_CHEP_REG_MASK | field)) ^ value) | USB_CHEP_VTRX | USB_CHEP_VTTX;
}

static inline void ep_rx_status(uint8_t ep, uint32_t status) {
  ep_toggle(ep, USB_CHEP_RX_STRX, status);
}

static inline void ep_tx_status(uint8_t ep, uint32_t status) {
  ep_toggle(ep, USB_CHEP_TX_STTX, status);
}

static inline void ep_clear_ctr_rx(uint8_t ep) {
  *epr(ep) = (*epr(ep) & USB_CHEP_REG_MASK & ~USB_CHEP_VTRX) | USB_CHEP_VTTX;
}

static inline void ep_clear_ctr_tx(uint8_t ep) {
  *epr(ep) = (*epr(ep) & USB_CHEP_REG_MASK & ~USB_CHEP_VTTX) | USB_CHEP_VTRX;
}

// Data toggle bits flip on a write of `1`, so a set bit is written back to clear it.
static void ep_clear_dtog(uint8_t ep, uint32_t dtog)
{
  if(*epr(ep) & dtog) ep_toggle(ep, 0, dtog);
}

static uint32_t ep_type_bits(USB_EpType_t type)
{
  switch(type) {
...
static void ep_activate(USB_t *usb, uint8_t ep)
{
  ...
  *epr(ep) = ep_type_bits(endpoint->type) | ep | USB_CHEP_VTRX | USB_CHEP_VTTX;
  USB_DRD_PMA_BUFF[ep].TXBD = endpoint->_pma_tx;
  USB_DRD_PMA_BUFF[ep].RXBD = pma_rx_blocks(endpoint->size) | endpoint->_pma_rx;
  endpoint->_tx_busy = false;
  ep_clear_dtog(ep, USB_CHEP_DTOG_TX);
  ep_clear_dtog(ep, USB_CHEP_DTOG_RX);
...
static void tx_load(USB_t *usb, uint8_t ep)
{
  ...
  USB_DRD_PMA_BUFF[ep].TXBD = ((uint32_t)len << PMA_TX_COUNT_Pos) | endpoint->_pma_tx;
...
static void tx_done(USB_t *usb, uint8_t ep)
{
  ...
  if(!ep && usb->_address) {
    USB_DRD_FS->DADDR = USB_DADDR_EF | (usb->_address & 0x7Fu);
    usb->_address = 0;
  }
...
{
  USB_Ep_t *endpoint = &usb->ep[ep];
  uint16_t count = (USB_DRD_PMA_BUFF[ep].RXBD >> PMA_RX_COUNT_Pos) & PMA_COUNT_MASK;
  ep_clear_ctr_rx(ep);
  if(setup) {
...
static void transfer(USB_t *usb, uint8_t ep)
{
  ...
  if(ep >= USB_EP_LIMIT) {
    ...
    *epr(ep) = reg & USB_CHEP_REG_MASK & ~(USB_CHEP_VTRX | USB_CHEP_VTTX);
    usb->errors++;
    return;
  }
  if(reg & USB_CHEP_VTRX) rx_done(usb, ep, reg & USB_CHEP_SETUP);
  if(reg & USB_CHEP_VTTX) tx_done(usb, ep);
}

static void irq_handler(USB_t *usb)
{
  const uint32_t handled = USB_ISTR_CTR | USB_ISTR_RESET | USB_ISTR_SUSP | USB_ISTR_WKUP |
    USB_ISTR_ERR | USB_ISTR_PMAOVR;
  uint32_t istr;
  // `ISTR` flags clear on a write of `0`, a write of `1` leaves them.
  while((istr = USB_DRD_FS->ISTR) & handled) {
    if(istr & USB_ISTR_RESET) {
      USB_DRD_FS->ISTR = ~USB_ISTR_RESET;
      bus_reset(usb);
    }
    else if(istr & USB_ISTR_CTR) {
      transfer(usb, istr & USB_ISTR_IDN); // cleared together with the endpoint flags
    }
      ...
      USB_DRD_FS->ISTR = ~(USB_ISTR_ERR | USB_ISTR_PMAOVR);
      usb->errors++;
    }
  }
}
...
// Packet memory map: descriptors first, then one IN and one OUT buffer per used endpoint
static status_t pma_map(USB_t *usb)
{
  ...
  for(uint8_t ep = 0; ep < USB_EP_LIMIT; ep++) {
    ...
    if(!tx && !rx) continue;
    // 32-bit packet memory keeps every buffer word aligned.
    if(endpoint->size < 8 || endpoint->size > 64 || (endpoint->size & 3)) return ERR;
    if(tx) {
...
status_t USB_Init(USB_t *usb)
{
  ...
  usb->_address = 0;
  RCC_EnableUSB();
  // `PA11`/`PA12` feed the transceiver directly, GPIO stays in its reset state.
  ...
  USB_DRD_FS->ISTR = 0;
  USB_DRD_FS->DADDR = USB_DADDR_EF;
  USB_DRD_FS->CNTR = USB_CNTR_CTRM | USB_CNTR_RESETM | USB_CNTR_SUSPM | USB_CNTR_WKUPM |
    USB_CNTR_ERRM | USB_CNTR_PMAOVRM;
  IRQ_EnableUSB(usb->irq_priority, (IRQ_Handler_t)irq_handler, usb);
  usb->_init = true;
  USB_DRD_FS->BCDR |= USB_BCDR_DPPU; // attach: `DP` pull-up
  return OK;
}
...
bool USB_EpStalled(USB_t *usb, uint8_t ep, bool in)
{
  ...
  uint32_t reg = *epr(ep);
  if(in) return (reg & USB_CHEP_TX_STTX) == USB_EP_TX_STALL;
  return (reg & USB_CHEP_RX_STRX) == USB_EP_RX_STALL;
}
...
status_t USB_Send(USB_t *usb, uint8_t ep, const uint8_t *data, uint16_t len, bool zlp)
{
  ...
  __disable_irq();
  status_t ret = OK;
  uint32_t stat = *epr(ep) & USB_CHEP_TX_STTX;
