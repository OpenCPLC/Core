...
static uint8_t address_of(SHT4X_t *sht4x)
{
...
static uint16_t delay_of_ms(uint8_t mode)
{
  switch(mode) {
...
static bool bus_write(SHT4X_t *sht4x, uint8_t *data, uint16_t len)
{
  if(I2C_Master_Write(sht4x->i2c, address_of(sht4x), data, len)) return false;
  if(timeout(21 + len, WAIT_&I2C_Master_IsFree, sht4x->i2c)) return false;
  return !I2C_Master_Nack(sht4x->i2c);
}

static bool bus_read(SHT4X_t *sht4x, uint8_t *data, uint16_t len)
{
  // A refused start leaves `_busy` untouched, and the wait below would read it as ours
  if(I2C_Master_Read(sht4x->i2c, address_of(sht4x), data, len)) return false;
  if(timeout(21 + len, WAIT_&I2C_Master_IsFree, sht4x->i2c)) return false;
  return !I2C_Master_Nack(sht4x->i2c);
}

// Response holds temperature first, then humidity, each with its own CRC byte
static bool check_append(SHT4X_t *sht4x)
{
...
bool SHT4X_Command(SHT4X_t *sht4x, SHT4X_CMD_t cmd)
{
  ...
  return bus_write(sht4x, &value, 1);
}
...
status_t SHT4X_Loop(SHT4X_t *sht4x)
{
  ...
  for(uint16_t i = 0; i < sht4x->oversampling; i++) {
    ...
    if(!bus_write(sht4x, &cmd, 1)) return ERR;
    delay(delay_of_ms(sht4x->mode));
    if(!bus_read(sht4x, sht4x->_buff_rx, 6)) return ERR;
    if(!check_append(sht4x)) return ERR;
  }
  calculate(sht4x);
